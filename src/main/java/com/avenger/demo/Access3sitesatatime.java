...
public class Access3sitesatatime {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        if (jmeterHome.exists()) {
            ...
            if (jmeterProperties.exists()) {
                ...
                // First HTTP Sampler - open Amazon
                HTTPSamplerProxy amazon = new HTTPSamplerProxy();
                amazon.setDomain("www.amazon.in");
                amazon.setPort(80);
                amazon.setPath("/");
                amazon.setMethod("GET");
                amazon.setName("Amazon");
                amazon.setProperty(TestElement.TEST_CLASS, HTTPSamplerProxy.class.getName());
                amazon.setProperty(TestElement.GUI_CLASS, HttpTestSampleGui.class.getName());


                // Second HTTP Sampler - open Flipkart
                HTTPSamplerProxy flipkart = new HTTPSamplerProxy();
                flipkart.setDomain("www.flipkart.com");
                flipkart.setPort(80);
                flipkart.setPath("/");
                flipkart.setMethod("GET");
                flipkart.setName("Flipkart");
                flipkart.setProperty(TestElement.TEST_CLASS, HTTPSamplerProxy.class.getName());
                flipkart.setProperty(TestElement.GUI_CLASS, HttpTestSampleGui.class.getName());

                // Second HTTP Sampler - open mythra
                HTTPSamplerProxy mythra = new HTTPSamplerProxy();
                mythra.setDomain("www.myntra.com");
                mythra.setPort(80);
                mythra.setPath("/");
                mythra.setMethod("GET");
                mythra.setName("Mynthra");
                mythra.setProperty(TestElement.TEST_CLASS, HTTPSamplerProxy.class.getName());
                mythra.setProperty(TestElement.GUI_CLASS, HttpTestSampleGui.class.getName());
                // Loop Controller
                ...
                //threadGroupHashTree.add(transactionController , flipkart);
                //threadGroupHashTree.add(transactionController, amazon);

                // 2nd method to initialize Transaction Controller with multiple Samples
                //.add(transactionController, Arrays.asList(flipkart,pauseAction, amazon,pauseAction,mythra));

                threadGroupHashTree.add(flipkart);
                threadGroupHashTree.add(pauseAction);
                threadGroupHashTree.add(amazon);
                threadGroupHashTree.add(pauseAction);
                threadGroupHashTree.add(mythra);
                threadGroupHashTree.add(ifController).add(flipkart);

                // save generated test plan to JMeter's .jmx file format
                SaveService.saveTree(testPlanTree, new FileOutputStream(jmeterHome + slash + "shopping.jmx"));
